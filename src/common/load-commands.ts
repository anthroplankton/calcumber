...
export async function loadCommandModules(commandModuleNames?: CommandModuleName[]) {
    ...
    return await Promise.all(
        commandModuleNames.map(
            name =>
                new Promise<Record<string, unknown>>(res =>
                    // unhandle the exception during the importing so that go on importing the other modules
                    import(path.format({ dir: '../commands', name, ext: '.js' })).then(res).finally(() => res({}))
                )
        )
    )
}
